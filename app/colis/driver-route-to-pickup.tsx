...
export default function DriverRouteToPickupScreen() {
  ...
  useEffect(() => {
    const calculateETA = async () => {
      if (!driverLocation || !parcel?.departureLocation) {
      ...
      try {
        const { data, error } = await supabase.functions.invoke('google-places-proxy', {
          body: {
            ...
            destinations: `${parcel.departureLocation.lat},${parcel.departureLocation.lng}`,
            mode: 'driving',
            language: 'fr',
          },
        });
  ...
  const handleOpenGoogleMaps = () => {
    if (!parcel?.departureLocation) {
      Alert.alert('Erreur', 'Adresse de départ non disponible');
      return;
    }

    const url = Platform.select({
      ios: `maps://app?daddr=${parcel.departureLocation.lat},${parcel.departureLocation.lng}`,
      android: `google.navigation:q=${parcel.departureLocation.lat},${parcel.departureLocation.lng}`,
      default: `https://www.google.com/maps/dir/?api=1&destination=${parcel.departureLocation.lat},${parcel.departureLocation.lng}`,
    });
    ...
    Linking.canOpenURL(url).then((supported) => {
      if (supported) {
        ...
        // Fallback to web URL
        const webUrl = `https://www.google.com/maps/dir/?api=1&destination=${parcel.departureLocation.lat},${parcel.departureLocation.lng}`;
  ...
  const handlePickedUp = async () => {
    ...
    Alert.alert(
      ...
      'Avez-vous bien récupéré le colis chez l\'expéditeur ?',
      [
        {
          text: 'Annuler',
          style: 'cancel',
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              await updateParcelStatus(parcelId, 'picked_up');
              ...
              Alert.alert(
                '✅ Colis récupéré',
                'Le colis a été récupéré avec succès. Direction le destinataire !',
                [
                  {
                    text: 'OK',
                    onPress: () => {
                      // Navigate to route to delivery screen
                      router.replace({
                        pathname: '/colis/driver-route-to-delivery',
                        params: { parcelId },
                      });
                    },
                  },
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? colors.darkBackground : colors.background }]}>
      ...
      >
        ...
        <View style={[styles.header, { backgroundColor: colors.primary }]}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={styles.headerTitle}>🚗 Trajet 1 : Récupération</Text>
            <Text style={styles.headerSubtitle}>Direction l&apos;expéditeur</Text>
          </View>
        </View>
        ...
        <View style={styles.content}>
          ...
          <View style={[styles.card, { backgroundColor: isDark ? colors.darkCard : colors.card }]}>
            ...
            {parcel.pricing && (
              <View style={[styles.distanceRow, { backgroundColor: isDark ? colors.darkBackground : colors.background }]}>
                <IconSymbol
                ...
                />
                <Text style={[styles.distanceText, { color: isDark ? colors.darkTextSecondary : colors.textSecondary }]}>
                  Distance totale : {parcel.pricing.distance.toFixed(1)} km
                </Text>
              </View>
            )}
          </View>
      ...
      <View style={[styles.actionButtonContainer, { backgroundColor: isDark ? colors.darkCard : colors.card }]}>
        <TouchableOpacity
          style={[styles.pickedUpButton, { backgroundColor: colors.primary }]}
        ...
        >
          ...
          <Text style={styles.pickedUpButtonText}>
            {isPickingUp ? 'CONFIRMATION...' : 'J\'AI RÉCUPÉRÉ LE COLIS'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
